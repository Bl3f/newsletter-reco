...
def get_activities(
    ...
    while keep_going:
        ...
        for activity in activities:
            ...
            cursor = min([datetime.strptime(activity["data"]["created_at"], "%Y-%m-%dT%H:%M:%S.%fZ"), cursor])
            yield activity

        print(created_at.start_value)
        print(cursor)

        if len(activities) < limit or (created_at.start_value is not None and cursor < created_at.start_value):
            keep_going = False
